...
contract Paycrest is IPaycrest, PaycrestSettingManager {
    ...
    // keccak256("createOrder(address _token,uint256 _amount,address _refundAddress,uint96 _rate,bytes32 _code)")
    ...
    function _handler(
        ...
        if (supportedInstitutionsByCode[_code] == bytes32(0))
            revert InvalidInstitutionCode();
    }
        ...
        )
    {
        ...
        primaryValidatorReward =
            (protocolFee * primaryValidatorFeePercent) /
            MAX_BPS;
        // get primary validators fees secondaryValidatorsReward
        secondaryValidatorsReward =
            (protocolFee * secondaryValidatorFeePercent) /
            MAX_BPS;
        // update new protocol fee
        protocolFee =
            protocolFee -
            (primaryValidatorReward + secondaryValidatorsReward);
    }
    ...
    function getFeeDetails()
        external
        view
        returns (uint64, uint64, uint64, uint256)
    {
        return (
            protocolFeePercent,
