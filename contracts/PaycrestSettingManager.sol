...
contract PaycrestSettingManager is Ownable {
    ...
    event SettingManagerForInstitution(
        bytes32 what,
        bytes8 value,
        bytes8 status
    );
    event PaycrestFees(
        uint64 protocolFee,
        uint64 primaryValidator,
        uint64 secondaryValidator
    );
    ...
    function setSupportedInstitutions(
        ...
        for (uint i = 0; i < length; ) {
            ...
            supportedInstitutionsByCode[institutions[i].code] = institutions[i]
                .name;
            unchecked {
                i++;
            }
    ...
    function updateProtocolFees(
        uint64 _protocolFee,
        uint64 _primaryValidator,
        uint64 _secondaryValidator
    ) external onlyOwner {
