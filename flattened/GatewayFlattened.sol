...
library StringsUpgradeable {
	...
	function toString(int256 value) internal pure returns (string memory) {
		return
			string(
				abi.encodePacked(value < 0 ? '-' : '', toString(SignedMathUpgradeable.abs(value)))
			);
	}
...
// File @openzeppelin/contracts-upgradeable/utils/cryptography/ECDSAUpgradeable.sol@v4.9.5
...
/**
 * @title IGateway
 * @dev Interface for the Gateway contract.
 */
interface IGateway {
	/* ##################################################################
                                EVENTS
	 ...
	 * - `_primaryValidator` must have stake on the Gateway staking platform.
	 * - `_secondaryValidators` must have stake on the Gateway staking platform.
	 * - `amount` must be greater than minimum.
	 * - `_refundable` refundable address must not be zero address.
...
contract GatewaySettingManager is OwnableUpgradeable {
...
/**
 * @title Gateway
 * @dev Gateway contract for handling orders and settlements.
 */
